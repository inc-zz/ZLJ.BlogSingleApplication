...
namespace Blogs.Infrastructure.Services.App
{
    ...
    /// 
    /// </summary>
    public class AppOpenIddictService : IAppOpenIddictService
    {
        ...
        private ILogger<AppOpenIddictService> _logger;

        public AppOpenIddictService(
            IConnectionMultiplexer redis,
            IConfiguration configuration,
            IHttpContextAccessor httpContextAccessor,
            IOptions<JwtConfig> jwtConfigOptions,
            ILogger<AppOpenIddictService> logger)
        {
        ...
        /// <returns></returns>
        public async Task<TokenResult> GenerateTokenAsync(BlogsUser user, string refreshToken = "")
        {
            try
            {
                // 创建身份主体
                ...
                identity.AddClaim(OpenIddictConstants.Claims.Name, user.Account);
                identity.AddClaim(OpenIddictConstants.Claims.PreferredUsername, user.Account);

                // 添加角色声明
                identity.AddClaim(OpenIddictConstants.Claims.Role, "user");
        ...
        public UserTokenModel JwtTokenParser(string token)
        {
            ...
            var jwtConfig = _configuration.GetSection("AppJwtConfig");
            ...
            {
                throw new ArgumentNullException("AppJwtConfig 配置不完整");
            }
        ...
        /// <returns></returns>
        private string GenerateAccessTokenManually(ClaimsPrincipal principal, BlogsUser user)
        {
            try
            {
                var handler = new JwtSecurityTokenHandler();
                var key = Encoding.UTF8.GetBytes(_configuration["AppJwtConfig:SecurityKey"]);
                ...
                var tokenDescriptor = new SecurityTokenDescriptor
                {
                    ...
                    Expires = expires,
                    Issuer = _configuration["AppJwtConfig:Issuer"],
                    Audience = _configuration["AppJwtConfig:Audience"],
        ...
        private async Task StoreTokensInRedisAsync(ClaimsPrincipal principal, string userId, string accessToken, string refreshToken)
        {
            ...
            var userTokenKey = $"appuser_tokens:{userId}";
